...
# ==================== CONFIG ====================
BOT_TOKEN = "8664037209:AAGJuTO4TJ29KOUlTCx6ZUCykzlV89kWDaM"
ADMIN_ID = 7500110150
...
VEHICLE_INFO_RACK_URL = 'https://vehicleinfo-byrack.vercel.app/api'
...
TOTAL_SEARCHES = 0
TOTAL_SEARCHES_LOCK = threading.Lock()

def increment_searches():
    global TOTAL_SEARCHES
    with TOTAL_SEARCHES_LOCK:
        TOTAL_SEARCHES += 1
...
def format_vehicle_info_rack(data):
    if not data or not data.get('response'):
        return "❌ No data found"
    resp = data.get('response', {})
    reg_no = resp.get('regNo', 'N/A')
    rto_data = resp.get('rtoData', {})
    rto_name = rto_data.get('rtoName', 'N/A')
    rto_code = rto_data.get('rtoCode', 'N/A')
    reg_authority = resp.get('regAuthority', 'N/A')
    chassis = resp.get('chassis', 'N/A')
    engine = resp.get('engine', 'N/A')
    reg_date = resp.get('regDate', 'N/A')
    manufacturer = resp.get('manufacturer', 'N/A')
    vehicle = resp.get('vehicle', 'N/A')
    vehicle_type = resp.get('vehicleType', 'N/A')
    variant = resp.get('variant', 'N/A')
    fuel_type = resp.get('fuelType', 'N/A')
    cubic_capacity = resp.get('cubicCapacity', 'N/A')
    seat_capacity = resp.get('seatCapacity', 'N/A')
    owner = resp.get('owner', 'N/A')
    financer_name = resp.get('financerName', 'N/A')
    insurance_company = resp.get('insuranceCompanyName', 'N/A')
    insurance_policy = resp.get('insurancePolicyNumber', 'N/A')
    insurance_upto = resp.get('insuranceUpto', 'N/A')
    pucc_number = resp.get('puccNumber', 'N/A')
    pucc_valid_upto = resp.get('puccValidUpto', 'N/A')
    present_address = resp.get('presentAddress', 'N/A')
    perm_address = resp.get('permAddress', 'N/A')
    pincode = resp.get('pincode', 'N/A')
    out = f"""
🚗 Vehicle: {reg_no}
🏢 RTO: {rto_name} ({rto_code})
...
def format_num_advance(data):
    ...
    if data.get('success') == False:
        return "❌ API returned failure. Number might be invalid or key expired."
    ...
    if calltracer:
        ...
        out += f"📱 SIM Card   : {calltracer.get('SIM card', 'N/A')}\n"
        out += f"📍 Mobile State: {calltracer.get('Mobile State', 'N/A')}\n"
        out += f"🔢 IMEI       : {calltracer.get('IMEI number', 'N/A')}\n"
        out += f"📶 MAC Address: {calltracer.get('MAC address', 'N/A')}\n"
        out += f"📶 Connection : {calltracer.get('Connection', 'N/A')}\n"
        out += f"🌐 IP Address : {calltracer.get('IP address', 'N/A')}\n"
        out += f"📍 Owner Addr : {calltracer.get('Owner Address', 'N/A')}\n"
        out += f"🏠 Hometown   : {calltracer.get('Hometown', 'N/A')}\n"
        out += f"🌍 Country    : {calltracer.get('Country', 'N/A')}\n"
        out += f"📍 Reference  : {calltracer.get('Refrence City', 'N/A')}\n"
        out += f"🔮 Personality: {calltracer.get('Owner Personality', 'N/A')}\n"
        out += f"🗣️ Language   : {calltracer.get('Language', 'N/A')}\n"
        out += f"📍 Locations  : {calltracer.get('Mobile Locations', 'N/A')}\n"
        out += f"📊 Complaints : {calltracer.get('Complaints', 'N/A')}\n"
        out += "\n"
    if records:
        ...
        for i, record in enumerate(records, 1):
            ...
            alt_numbers = []
            for field in ['Phone2','Phone3','Phone4','Phone5','Phone6','Phone7','Phone8','Phone9','Phone10']:
                val = record.get(field)
                if val and val not in ['0','N/A','','None']:
                    alt_numbers.append(str(val))
            if alt_numbers:
                out += "📱 Alt Nos   :\n"
                for idx, alt in enumerate(alt_numbers, 1):
                    out += f"               {idx}. {alt}\n"
            if record.get('FullName'):
                out += f"👤 Full Name  : {record.get('FullName')}\n"
            if record.get('FatherName'):
                out += f"👤 Father     : {record.get('FatherName')}\n"
            all_addresses = []
            for field in ['Adres','Adres2','Adres3','Adres4','Adres5','Adres6','Adres7','Adres8']:
                val = record.get(field)
                if val and val not in ['!','N/A','','None']:
                    addr_clean = str(val).replace('!', ', ').strip()
                    if addr_clean and addr_clean not in all_addresses:
                        all_addresses.append(addr_clean)
            if all_addresses:
                out += "📍 Address    :\n"
            ...
            if record.get('Region'):
                out += f"📶 Region     : {record.get('Region')}\n"
            if i < total_records:
                out += "\n" + "─" * 35 + "\n"
    out += "\n" + "═" * 40 + "\n"
    out += f"📅 GENERATED: {datetime.now().strftime('%d-%b-%Y %I:%M %p')}\n"
...
TEXTS = {
    'en': {
        ...
        'ask_vehicle1': '🚗 Send vehicle number for Rack API (e.g. RJ14CV0002):',
        'ask_veh2num': '🚗 Send vehicle number for details:',
        'ask_tgid': '📱 Send Telegram User ID or @username:',
    ...
    'hi': {
        ...
        'ask_vehicle1': '🚗 Rack API के लिए वाहन नंबर भेजें (जैसे RJ14CV0002):',
        'ask_veh2num': '🚗 वाहन नंबर से डिटेल्स पाने के लिए भेजें:',
        'ask_tgid': '📱 Telegram User ID या @username भेजें:',
...
def send_main_menu(chat_id, user_id, lang='en'):
    ...
    keyboard.add(telebot.types.KeyboardButton("🚗 Vehicle Info 1"))
    keyboard.add(telebot.types.KeyboardButton("🔗 Vehicle to Number"))
    keyboard.add(telebot.types.KeyboardButton("🆔 TG USERNAME & USER ID TO NUMBER"))
    keyboard.add(telebot.types.KeyboardButton("💥 SMS BOMBER"))
    ...
    keyboard.add(telebot.types.KeyboardButton("📊 Bot Status"))
    if user_id == ADMIN_ID:
...
# ==================== BOT STATUS ====================
def bot_status_handler(message):
    ...
    status_msg = f"""
{emoji} <b>🤖 BOT STATUS</b> {emoji}

<b>📊 General Info:</b>
• Status: <b>{api_status}</b>
• Uptime: <code>{uptime_str}</code>
• Total Users: <code>{total_users}</code>
• Banned Users: <code>{banned_users}</code>
• Promo Codes: <code>{total_promos}</code>

<b>🔍 Search Stats:</b>
• Total Searches: <code>{TOTAL_SEARCHES}</code>
• Admin Balance: <code>♾️ Unlimited</code>

<b>⚡ Performance:</b>
• Response Time: <code>~0.5s</code>
• Database: <code>✅ Connected</code>
• Telethon: <code>✅ Active</code>

<b>🛡️ Security:</b>
• Verification: <code>✅ Enabled</code>
• Rate Limit: <code>✅ Active</code>
...
def admin_panel_handler(message):
    ...
    keyboard.add(telebot.types.KeyboardButton("🤖 AI Assistant"))
...
@bot.message_handler(func=lambda m: m.text == "🤖 AI Assistant" and m.from_user.id == ADMIN_ID)
def ai_assistant_button(message):
    user_id = message.from_user.id
    user_states[user_id] = "waiting_ai_query"
    bot.reply_to(message, "🤖 **Ask me anything, master!**\n\nSend your query and I'll reply with the help of AI.", parse_mode='Markdown')
...
# ==================== BOMBER (SMS ONLY) ====================
def bomber_worker(user_id, phone, total_rounds):
    ...
    try:
        bot.edit_message_text(
            f"💣 SMS Bombing started on {phone}!\n"
            f"📦 Total rounds: {total_rounds}\n"
            f"📨 10 SMS per round = {total_rounds*10} SMS\n"
            f"🎯 Target: {total_rounds*10} SMS\n\n"
            f"⏳ Round 0/{total_rounds} - Starting...",
            chat_id=user_id,
    ...
    final_msg = (
        ...
        f"📦 Total rounds: {current_round}/{total_rounds}\n"
        f"🎯 Target: {total_rounds*10} SMS"
    )
...
def start_bomber_process(message, user_id, phone, rounds):
    if user_id in bombing_sessions:
    ...
    msg = bot.reply_to(
        message,
        f"💣 SMS Bomber started on {phone}!\n"
        f"📦 {rounds} rounds\n"
        f"📨 10 SMS per round = {rounds*10} SMS\n\n"
        f"⏳ Estimated time: {rounds*1.5} seconds\n"
        f"🛑 Click STOP to end bombing.",
    ...
    threading.Thread(target=bomber_worker, args=(user_id, phone, rounds), daemon=True).start()
...
@bot.message_handler(commands=['vehicle1'])
def vehicle1_command(message):
    user_id = message.from_user.id
    lang = get_lang(user_id)
    msg = bot.reply_to(message, TEXTS[lang]['ask_vehicle1'])
    user_states[user_id] = "waiting_vehicle1"
    register_user(user_id)
...
def handle_all(message):
    ...
    # ==================== AI ASSISTANT (Pure Chatbot - No special commands) ====================
    if state == "waiting_ai_query" and user_id == ADMIN_ID:
        try:
            resp = requests.get(f"https://api.popcat.xyz/chatbot?msg={text}", timeout=10)
            if resp.status_code == 200:
                data = resp.json()
                answer = data.get("response", "Sorry, I couldn't understand that.")
            else:
                answer = f"⚠️ AI service returned error code {resp.status_code}. Please try again later."
        except Exception as e:
            answer = f"❌ Error: {str(e)}. Please check your internet or try again."
    ...
    menu_map = {
        ...
        "Vehicle Info 1": "waiting_vehicle1",
        "Vehicle to Number": "waiting_veh2num", "TG USERNAME & USER ID TO NUMBER": "waiting_tgid",
        "SMS BOMBER": "waiting_bomber", "UPI ID Finder": "waiting_upi",
        "IP Info": "waiting_ip", "GST Info": "waiting_gst",
        "IFSC Info": "waiting_ifsc", "Bot Status": "bot_status",
    }
    ...
    if clean_text in menu_map:
        ...
        elif action == "bot_status": bot_status_handler(message)
    ...
    # ---- Search Limit ----
    if user_id != ADMIN_ID and user_id != PARTNER_ID:
        if not can_search(user_id) and state not in ['waiting_bomber', 'waiting_bomber_rounds']:
            remaining = get_remaining_tries(user_id)
    ...
    if state == "waiting_num_advance":
        if text.isdigit() and len(text) == 10:
            ...
            try:
                resp = requests.get(f"{NUM_ADVANCE_URL}?key={NUM_ADVANCE_KEY}&num={text}", timeout=30)
                data = resp.json()
    ...
    if state == "waiting_vehicle":
        ...
        if len(text_upper) >= 6:
            ...
            found = False
            try:
                resp_backup = requests.get(f"https://reseller-host.vercel.app/api/rc?number={text_upper}", timeout=30)
                ...
                print(f"Reseller API Error: {e}")
            if not found:
                try:
                    resp = requests.get(f"{VEHICLE_INFO_URL}?num={text_upper}", timeout=30)
                    if resp.status_code == 200:
                        data = resp.json()
                        if data and data.get('api_1_car_info'):
                            raw_data = data
                            result_text = format_vehicle_info(data)
                            found = True
                except Exception as e:
                    print(f"Bronx API Error: {e}")
            if result_text:
                if user_id != ADMIN_ID and user_id != PARTNER_ID:
                    use_try(user_id)
                increment_searches()
                ...
                safe_delete_message(message.chat.id, loading_msg.message_id)
                bot.reply_to(message, "❌ No vehicle data found from any source.")
            user_states[user_id] = None
        else:
            bot.reply_to(message, TEXTS[lang]['err_vehicle'])
        return

    if state == "waiting_vehicle1":
        text_upper = text.upper().strip()
        if len(text_upper) >= 6:
            loading_msg = bot.reply_to(message, "⏳ Fetching vehicle data from Rack API...")
            try:
                resp = requests.get(f"{VEHICLE_INFO_RACK_URL}?search={text_upper}", timeout=30)
                if resp.status_code == 200:
                    data = resp.json()
                    if data and data.get('response'):
                        result_text = format_vehicle_info_rack(data)
                        if user_id != ADMIN_ID and user_id != PARTNER_ID:
                            use_try(user_id)
                        increment_searches()
                        send_result_with_buttons(message.chat.id, loading_msg, result_text, data, user_id)
                        ...
                        safe_delete_message(message.chat.id, loading_msg.message_id)
                        bot.reply_to(message, "❌ No vehicle data found for this number.")
                else:
                    safe_delete_message(message.chat.id, loading_msg.message_id)
    ...
    if state == "waiting_veh2num":
        ...
        if len(text_upper) >= 6:
            loading_msg = bot.reply_to(message, "⏳ Processing...")
            try:
                resp = requests.get(f"{VEHICLE_TO_NUM_URL}?key={VEHICLE_TO_NUM_KEY}&vehicle={text_upper}", timeout=30)
                data = resp.json()
    ...
    if state == "waiting_tgid":
        ...
        if text.isdigit():
            ...
            if resolved_id:
                ...
                bot.edit_message_text(
                    f"❌ Username `{text}` not found or invalid.\n\n💡 Make sure the username is public.\n🔄 Alternatively, send the numeric User ID directly.",
        ...
        if target_id:
            try:
                ...
                if resp.status_code != 200:
                    ...
                    bot.edit_message_text(
                        f"❌ API Connection Error!\n\n{error_detail}\n\n💡 The API server might have blocked your hosting IP temporarily. Try again in 5-10 minutes.",
                        chat_id=message.chat.id,
                        message_id=loading_msg.message_id
                    )
                    user_states[user_id] = None
                ...
                if data.get('status'):
                    ...
                    bot.edit_message_text(
                        f"❌ API returned status false.\n\nFull Response:\n{json.dumps(data, indent=2)}",
                        chat_id=message.chat.id,
                        message_id=loading_msg.message_id
                    )
            except Exception as e:
                bot.edit_message_text(
                    f"❌ Request Error: {e}\n\n💡 Check if the API server is down or your hosting IP is blocked.",
                    chat_id=message.chat.id,
                    message_id=loading_msg.message_id
                )
        user_states[user_id] = None
    ...
    if state == "waiting_bomber":
        if text.isdigit() and len(text) == 10:
            ...
            user_states[user_id] = "waiting_bomber_rounds"
            bot.reply_to(
                message,
                f"📱 Number: {text}\n\n"
                "❓ How many rounds do you want to send?\n"
                "⚠️ Each round = 10 SMS\n"
                "📌 Enter a number (max 1000 rounds):"
            )
        else:
            bot.reply_to(message, TEXTS[lang]['err_num'])
        return

    if state == "waiting_bomber_rounds":
        if text.isdigit():
            rounds = int(text)
            if 1 <= rounds <= 1000:
                phone = temp_data.get(user_id, {}).get('phone')
                if phone:
                    start_bomber_process(message, user_id, phone, rounds)
                else:
                    bot.reply_to(message, "❌ Session expired. Please start again.")
                user_states[user_id] = None
                temp_data.pop(user_id, None)
            else:
                bot.reply_to(message, "❌ Please enter a number between 1 and 1000.")
        else:
            bot.reply_to(message, "❌ Invalid input. Send a number (e.g., 50).")
        return
    ...
    if state == "waiting_ip":
        if len(text.split('.')) == 4 or ':' in text:
            ...
            try:
                resp = requests.get(f"{IP_INFO_URL}?ip={text}", timeout=30)
                data = resp.json()
    ...
    if state == "waiting_gst":
        if len(text) == 15:
            ...
            try:
                resp = requests.get(f"{GST_INFO_URL}?gstin={text}", timeout=30)
                data = resp.json()
    ...
    if state == "waiting_ifsc":
        if len(text) >= 4:
            ...
            try:
                resp = requests.get(f"{IFSC_INFO_URL}{text}", timeout=30)
                data = resp.json()
