...
# ==================== CONFIG ====================
# Read bot token from environment variable (Railway) or fallback to hardcoded token
BOT_TOKEN = os.environ.get('BOT_TOKEN', '8664037209:AAHqed22av7jo2n6hVTEapArSMqt7eNbR8s')
ADMIN_ID = 7500110150
...
# ==================== TELETHON SETUP (with session from env or file) ====================
session_file = 'session_name.session'

# If RAILWAY_SESSION env var exists, decode and write to session file
railway_session_b64 = os.environ.get('RAILWAY_SESSION')
if railway_session_b64:
    try:
        session_data = base64.b64decode(railway_session_b64)
        with open(session_file, 'wb') as f:
            f.write(session_data)
        print("✅ Session decoded from environment variable.")
    except Exception as e:
        print(f"⚠️ Failed to decode RAILWAY_SESSION: {e}")
...
def resolve_username_sync(username):
    ...
    try:
        ...
        return None, f"⚠️ Rate Limit! Wait {wait_time}s."
    except UsernameNotOccupiedError:
        return None, "❌ Username not found or not public."
    except Exception as e:
...
def stop_bomber_callback(call):
    bot.answer_callback_query(call.id, text="🚫 Bomber service is currently stopped by admin.", show_alert=True)
    try:
...
# ==================== BOMBER COMMAND (DISABLED) ====================
@bot.message_handler(commands=['bomber'])
def bomber_command(message):
    bot.reply_to(message, "🚫 SMS & CALL Bomber service is currently stopped by admin. This service will be available soon. (जल्द ही यह सेवा आएगी)")
...
def handle_all(message):
    ...
    menu_map_lower = {
        ...
        "num info": "waiting_num",
        "num info advance": "waiting_num_advance",
        "aadhar info": "waiting_aadhar",
        "vehicle info": "waiting_vehicle",
        "vehicle to number": "waiting_veh2num",
        "tg username & user id to number": "waiting_tgid",
        "sms + call bomber": "waiting_bomber",
        "upi id finder": "waiting_upi",
        "ip info": "waiting_ip",
        "gst info": "waiting_gst",
        "ifsc info": "waiting_ifsc",
        "history": "history",
        "clear history": "clear_history",
        "redeem promo code": "promo_code"
    }

    action = menu_map_lower.get(clean_text.lower())
    if action:
        user_states[user_id] = None
        if action == "owner":
            show_owner_details(message.chat.id, lang)
    ...
    # ==================== No Searches Left Check (skip for bomber) ====================
    if user_id != ADMIN_ID and user_id != PARTNER_ID:
    ...
    if state == "waiting_vehicle":
        ...
        if len(text_upper) >= 6:
            ...
            if not found:
                try:
                    resp2 = requests.get(f"{VEHICLE_INFO_URL_2}?vehicle_number={text_upper}")
                    if resp2.status_code == 200:
                        data2 = resp2.json()
                        if data2 and data2.get('api_1_car_info'):
                            raw_data = data2
                            result_text = format_vehicle_info(data2)
                            found = True
                except: pass
            if not found:
                try:
                    resp3 = requests.get(f"{VEHICLE_INFO_URL_3}?vehicle_number={text_upper}")
                    if resp3.status_code == 200:
                        data3 = resp3.json()
                        if data3 and data3.get('api_1_car_info'):
                            raw_data = data3
                            result_text = format_vehicle_info(data3)
                            found = True
                except: pass
            if not found:
                try:
    ...
    if state == "waiting_tgid":
        ...
        if text.isdigit():
            ...
            resolved_id, error_msg = resolve_username_sync(text)
            if resolved_id:
        ...
        if target_id:
            try:
                ...
                bot.edit_message_text(f"❌ Request Error: {e}", chat_id=message.chat.id, message_id=loading_msg.message_id)
        user_states[user_id] = None
        return
